...
def timestamp_pixel(accel_file, hr_file, out_path, participant_id):
    ...
    for file in hr_file:
        ...
        hr_list.append(
            pd.read_csv(file, header=None, names=['Counter', 'Time', 'Heart Rate'], usecols=[1, 2], parse_dates=[0],
...
def combine_pixel(accel_data, hr_data, out_path, participant_id, participant_age, start, end, protocol="PA"):
    ...
    final_df[["X", "Y", "Z"]] = final_df[["X", "Y", "Z"]].apply(lambda x: x / 9.8)
    # Get Second Fraction
    ...
    # Flag Heart Rate
    flagged_hr = flag_hr(final_df, "Pixel", participant_age, protocol=protocol)
    ...
    output_file = output_path + '/' + participant_id + '_pixel.csv'
    final_df.to_csv(output_file, index=False)

    return ["Pixel", final_df]