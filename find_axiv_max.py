...
AXIV_COL_NAMES = ["Timestamp", "x", "y", "z"]
TIME_STR = "%Y-%m-%d_%H:%M:%S"

def find_max_axiv(filename, start_time, end_time):
    df = pd.read_csv(filename, header=None, names=AXIV_COL_NAMES)
    #print(df)
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("usage: python", sys.argv[0], "<axiv_file> <start_time yyyy-mm-dd_HH:MM:SS> <end_time yyyy-mm-dd_HH:MM:SS>")
        sys.exit(1)
    ...
    rec = find_max_axiv(filename, start_time, end_time)
    print(rec)
